...
int main(int argc, char** argv)
{
	 ...
	 */	
//	freopen("sample_input_0.txt", "r", stdin); //erase // of #include <cstdio> too to use this
	...
	cout << "sorting Time : " << (end-begin)/(float)CLOCKS_PER_SEC << endl;

		for(int i=2; i<n+1; i++){
			if( not_range_k(mem[rankTable[bus.front()]],mem[i],k) ){
				rankTable[bus.front()] = i;
				bus.push_back(bus.front());
				bus.pop_front();
			}else if( not_range_k(mem[rankTable[bus.back()]],mem[i],k) ){
